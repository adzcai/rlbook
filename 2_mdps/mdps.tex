...
\For{$t = 0, \dots, T-1$}
    ...
    \Comment (Exact) Policy Evaluation (see \ref{eq:matrix_inversion_pe})
...
\begin{align*}
    ...
    &\ge \cdots
\end{align*}
which implies that $V^{\pi^{t+1}}(s) \ge V^{\pi^{t}}(s)$ for all $s$ (since the r.h.s. converges to zero). We can then plug this back into \ref{eq:pi_iter_proof} to get the desired result:
\begin{align*}
