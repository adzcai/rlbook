...
We can instead subtract a \textbf{baseline function} $b_t : \cS \to \R$ at each timestep $t.$ This modifies the policy gradient as follows: \[
...
More generally, we could map states and actions to unnormalized scores via some parameterized function $f_\theta : \cS \times \cA \to \R,$ such as a neural network, and choose actions according to a softmax: \[
