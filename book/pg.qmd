...
namely @sec-policy_iteration for finite MDPs and @sec-iterative_lqr in continuous control.

In particular, we often use policies that can be described by some finite set of **parameters.**
We will see some examples in @sec-parameterizations.
...
Remember that in RL, the primary goal is to find the _optimal policy_ that achieves the maximimum total reward, which we can express using @def-value:
...
This gives the following expression for the policy gradient, known as the "REINFORCE" policy gradient @williams_simple_1992:
...
and substituting these into the expression inside the brackets of @eq-reinforce_pg.
...
This quantity matches the intuition given in @lem-intuitive-remark:
...
Plugging this into the @thm-pdl gives
...
measured in terms of the r.h.s. of the @thm-pdl,
...
We recommend @boyd_convex_2004 for a comprehensive treatment of convex optimization.
...
where distance between distributions is measured by the @def-kld.
...
This is the objective of the **proximal policy optimization** algorithm (@schulman_proximal_2017).
...
We can once again use @sec-importance_sampling to rewrite the inner expectation:
...
culminating in the advantage-based expression @eq-pg_advantage.
...
and then saw ways to relax the constraint in @def-npg and @sec-ppo.
