...
:::{note} SGD convergence More formally, suppose we run SGD for $K$ steps, using an unbiased gradient estimator. Let the step size $\eta^k$ scale as $O(1/\sqrt{k}).$ Then if $J$ is bounded and $\beta$-smooth (see below), and the *norm* of the gradient estimator has a bounded second moment $\sigma^2,$
...
Remember that in RL, the primary goal is to find the *optimal policy* that achieves the maximimum total reward, which we can express using @def-value:
...
where $\rho^\pi$ is the distribution over trajectories induced by $\pi$ (see @def-autoregressive_trajectories).
...
If both the state and action spaces are finite, perhaps we could simply learn a preference value $\theta_{s,a}$ for each state-action pair. Then to turn this into a valid distribution, we perform a **softmax** operation: we exponentiate each of them, and then normalize to form a valid distribution.
...
More generally, we could map states and actions to unnormalized scores via some parameterized function $f_\theta : \mathcal{S} \times \mathcal{A} \to \mathbb{R},$ such as a neural network, and choose actions according to a softmax:
...
::: {#lem-intuitive-remark}
## Intuition behind REINFORCE

Intuitively speaking, we want to update the policy parameters to maximize the probability of taking *optimal actions*. That is, suppose we are in state $s$, and $a^\star$ is an optimal action to take. Then we want to solve $\theta = \arg\max_{\theta'} \pi_{\theta'}(a^\star \mid s)$, which would lead to the gradient ascent expression
...
A central idea from supervised learning is the **bias-variance decomposition**, which shows that the mean squared error of an estimator is the sum of its squared bias and its variance. The REINFORCE gradient estimator @eq-reinforce_pg is already *unbiased,* meaning that its expectation over trajectories is the true policy gradient. Can we find ways to reduce its *variance* as well?
...
We can further reduce variance by subtracting a **baseline function** $b_\hi : \mathcal{S} \to \mathbb{R}$ at each timestep $\hi$. This modifies the policy gradient as follows:
...
(Again, you should try to prove that this equality still holds.) For example, we might want $b_\hi$ to estimate the average reward-to-go at a given timestep:

$$
b_\hi^\theta = \E_{\tau \sim \rho_\theta} R_\hi(\tau).
$$

As a better baseline, we could instead choose the *value function.* Note that the random variable $Q^\pi_\hi(s, a) - V^\pi_\hi(s),$ where the randomness is taken over the actions, is centered around zero. (Recall $V^\pi_\hi(s) = \E_{a \sim \pi} Q^\pi_\hi(s, a).$) This quantity matches the intuition given in @lem-intuitive-remark: it is *positive* for actions that are better than average (in state $s$), and *negative* for actions that are worse than average. In fact, this quantity has a particular name: the **advantage function.**
...
The gradient-log-likelihood for the linear-in-features parameterization @exm-linear-in-features is also quite elegant:
...
What advantages does the policy gradient algorithm have over the policy iteration algorithms covered in @sec-policy_iteration?

:::{note} Policy iteration recap Recall that policy iteration is an algorithm for MDPs with unknown state transitions where we alternate between these two steps:
...
:::{image} shared/npg_line.png :alt: a line from (0, 1) to (1, 0) :width: 240px :align: center
:::
...
This should sound familiar: we want to estimate an expectation over one distribution by sampling from another. We can once again use @sec-importance_sampling to rewrite the inner expectation:
...
Policy gradient methods are a powerful family of algorithms that directly optimize the expected total reward by iteratively updating the policy parameters. Precisely, we estimate the gradient of the expected total reward (with respect to the parameters), and update the parameters in that direction. But estimating the gradient is a tricky task! We saw many ways to reduce the variance of the gradient estimator, culminating in the advantage-based expression @eq-pg_advantage.
