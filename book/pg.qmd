...
Typically in these cases, we compute some _estimate_ of the gradient at each step, and walk in that direction instead.
...
::::{prf:example} Policy gradient for the linear-in-features parameterization

The gradient-log-likelihood for the linear-in-features parameterization [](#linear-in-features) is also quite elegant:
...
The baseline estimation step `fit_value` can be done using any appropriate supervised learning algorithm.
