...
$$
\hat \pi = \arg\max_{\pi \in \Pi} \E_{\tau \sim \rho^\pi} [R(\tau)] \quad \text{where} \quad R(\tau) := \sum_{\step=0}^{\Step-1} r(s_\step, a_\step).
...
$$
R(\tau) := \sum_{\step=0}^{\Step-1} r_\step
$$ {#eq-total-reward}
...
\text{where}\quad J(\theta) &:= \E_{\tau \sim \rho^{\pi_\theta}} \sum_{\step=0}^{\Step-1} r(s_\step, a_\step).
\end{aligned}
...
\theta^{t+1} = \theta^t + \eta \nabla J(\theta^t)
\quad \text{where} \quad
J(\theta) = \E_{\tau \sim \rho^{\pi_\theta}} \left[ \sum_{\step=0}^{\Step-1} r(s_\step, a_\step) \right].
$$ {#eq-pg-optimization}
...
Suppose we use $\picurr$ to roll out a trajectory $\tau = (s_0, a_0, \dots, s_{\Step-1}, a_{\Step-1})$
and compute @eq-gd-log-step at each step of the trajectory.
We compute a gradient coefficient at each timestep,
so we denote each instance by $\psi_\step(\tau)$.

$$
g_\tau(\theta)
:=
\sum_{\step=0}^{\Step-1}
\psi_\step(\tau) \nabla \log \pi_{\theta}(a_\step \mid s_\step).
...
Suppose we are given an expression for the gradient coefficients $\psi_\step(\tau)$.
...
$\tau^n = (s^n_0, a^n_0, r^n_0, \dots, s^n_{\Step-1}, a^n_{\Step-1}, r^n_{\Step-1})$,
and compute the update rule

$$
\begin{aligned}
\theta^{t+1} &= \theta^t + \eta \frac{1}{N} \sum_{n=1}^N g_{\tau^n}(\theta^t) \\
\text{where} \quad g_{\tau}(\theta) &= \sum_{\step=0}^{\Step-1}
\psi_\step(\tau) \nabla \log \pi_{\theta}(a_\step \mid s_\step).
\end{aligned}
$$ {#eq-general-pg}
:::
...
$\psi_\step(\tau)$ should correlate with the quality of the action taken at time $\step$.
It may depend on the current policy $\picurr$
or any component of the trajectory $(s_0, a_0, r_0, \dots, s_{\Step-1}, a_{\Step-1}, r_{\Step-1})$.
:::

We won't keep you waiting:
it turns out that if we set $\psi_\step(\tau)$ to

1. $R(\tau)$ (the total reward of the trajectory) (@eq-estimator-reinforce),
1. $\sum_{\step'=\step}^{\Step-1} r(s_{\step'}, a_{\step'})$ (the remaining reward in the trajectory),
1. $Q^{\picurr}(s_\step, a_\step)$ (the current policy's Q-function), or
1. $A^{\picurr}(s_\step, a_\step)$ (the current policy's advantage function),
...
We begin by showing that setting $\psi_\step = \sum_{\step'=0}^{\Step-1} r_{\step'}$,
...
$$
\psi_\step(\tau) := R(\tau) := \sum_{\step'=0}^{\Step-1} r_{\step'}
...
(i.e. $s_{\step}$ only depends on $s_{\step-1}, a_{\step-1}$)
and the policy is history-independent
(i.e. $a_\step \sim \pi_\theta (\cdot \mid s_\step)$),
...
$$
\log \rho^{\pi_\theta}(\tau) = \log P_0(s_0) + \sum_{\step=0}^{\Step-1} \Big(
    \log \pi_\theta(a_\step \mid s_\step) + \log P(s_{\step+1} \mid s_\step, a_\step)
\Big)
$$ {#eq-log-autoregressive}

When we take the gradient with respect to the parameters $\theta$,
only the $\log \pi_\theta(a_\step \mid s_\step)$ terms depend on $\theta$:
...
As a first step,
note that the action taken at step $\step$
...
Substituting the reamining reward $\sum_{\step'=\step}^{\Step-1} r_{\step'}$
for $\psi_\step(\tau)$
into the general policy gradient estimator @eq-general-pg gives an unbiased estimator.
That is,
...
&:=
\sum_{\step=0}^{\Step-1}
\left( \sum_{\step'=\step}^{\Step-1} r_{\step'} \right)
\nabla \log \pi_{\theta}(a_\step \mid s_\step).
\end{aligned}
...
since the only stochasticity in the expression $Q^{\pi_\theta}(s_\step, a_\step)$
comes from the current state and action.

::: {#thm-unbiased-q}
#### Using the Q function is unbiased

Substituting $Q^{\pi_\theta}(s_\step, a_\step)$
for $\psi_\step(\tau)$
into the general policy gradient estimator @eq-general-pg gives an unbiased estimator.
That is,
...
$$
\psi^{\text{bl}}_\step(\tau) := \psi_\step(\tau) - b_\theta(s_\step)
...
For example, we might want $b_\step$ to estimate the average remaining reward at a given timestep:

$$
b_\theta(s_\step) = \E_{\tau \sim \rho^{\pi_\theta}} \left[
    \sum_{\step'=\step}^{\Step-1} r_{\step'}
\right].
$$ {#eq-exm-remaining}

As a better baseline, we could instead choose the *value function* of $\pi_{\theta}$.
For any policy $\pi$,
note that the random variable $Q^\pi_\step(s, a) - V^\pi_\step(s)$,
where the randomness is taken over the action $a$,
is centered around zero.
(Recall $V^\pi_\step(s) = \E_{a \sim \pi} Q^\pi_\step(s, a).$)
...
For a policy $\pi$,
its advantage function $A^\pi$ at time $\step$ is given by

$$
A^\pi_\step(s, a) := Q^\pi_\step(s, a) - V^\pi_\step(s).
$$ {#eq-advantage}
:::
...
We can now use $A^{\pi_{\theta}}(s_\step, a_\step)$
...
$$
\psi_\step(\tau) := A^{\pi_\theta}(s_\step, a_\step)
$$ {#eq-estimator-adv}
...
\begin{aligned}
    ...
    \sum_{t=0}^{T-1} \nabla \log \pi_\theta(a_\step | s_\step) A_\step^{\pi_\theta}
    \right]                                                                                                                    \\
                     & = \E_{\tau \sim \rho^{\pi_\theta}} \left[
    \sum_{t=0}^{T-1} \left( \phi(s_\step, a_\step) - \E_{a' \sim \pi(s_\step)} \phi(s_\step, a') \right) A_\step^{\pi_\theta}(s_\step, a_\step)
    \right]                                                                                                                    \\
                     & = \E_{\tau \sim \rho^{\pi_\theta}} \left[ \sum_{t=0}^{T-1} \phi(s_\step, a_\step) A_\step^{\pi_\theta} (s_\step, a_\step) \right]
\end{aligned}
$$

Why can we drop the $\E \phi(s_\step, a')$ term? By linearity of expectation, consider the dropped term at a single timestep: $\E_{\tau \sim \rho^{\pi_\theta}} \left[ \left( \E_{a' \sim \pi(s_\step)} \phi(s, a') \right) A_\step^{\pi_\theta}(s_\step, a_\step) \right].$ By Adam's Law, we can wrap the advantage term in a conditional expectation on the state $s_\step.$ Then we already know that $\E_{a \sim \pi(s)} A_\step^{\pi}(s, a) = 0,$ and so this entire term vanishes.
:::
...
Suppose Alice is playing a game (an MDP). Bob is spectating, and can evaluate how good an action is compared to his own strategy. (That is, Bob can compute his *advantage function* $A_\step^{\text{Bob}}(s_\step, a_\step)$). The performance difference lemma says that Bob can now calculate exactly how much better or worse he is than Alice as follows:

$$
V_0^{\text{Alice}}(s) - V_0^{\text{Bob}}(s) = \E_{\tau \sim \rho^\text{Alice}} \left[ \sum_{\step=0}^{\Step-1} A_\step^{\text{Bob}} (s_\step, a_\step) \mid s_0 = s \right]
$$ {#eq-pdl}

where $\rho^\text{Alice}$ denotes Alice's trajectory distribution (@def-trajectory-distribution).
:::

To see why, consider a specific step $\step$ in the trajectory. We compute how much better actions from Bob are than the actions from Alice, on average. But this is exactly the average Bob-advantage across actions from Alice, as described in the PDL!
...
$$
\begin{aligned}
A^\pi_\step(s_\step, a_\step) &= Q^\pi_\step(s_\step, a_\step) - V^\pi_\step(s_\step) \\
&= r_\step(s_\step, a_\step) + \E_{s_{\step+1} \sim P(\cdot \mid s_\step, a_\step)} [V^\pi_{\step+1}(s_{\step+1})] - V^\pi_\step(s_\step)
\end{aligned}
...
$$
\begin{aligned}
\E_{\tau \sim \rho^\text{Alice}} \left[ \sum_{\step=0}^{\Step-1} A_\step^{\text{Bob}} (s_\step, a_\step) \mid s_0 = s \right]
&= \E_{\tau \sim \rho_\text{Alice}} \bigg[
    \left( \sum_{\step=0}^{\Step-1} r_\step(s_\step, a_\step) \right) \\
& \quad {} + \left( V^{\text{Bob}}_1(s_1) + \cdots + V^{\text{Bob}}_\Step(s_\Step) \right) \\
& \quad {} - \left( V^{\text{Bob}}_0(s_0) + \cdots + V^{\text{Bob}}_{\Step-1}(s_{\Step-1}) \right)
    \mid s_0 = s \bigg] \\
...
$$
\Delta_\infty = \min_{s \in \mathcal{S}} A^{\pi}_\step(s, \widetilde \pi(s)).
...
\sum_{\step=0}^{\Step-1} A_\step^{\pi}(s_\step, a_\step)
\mid s_0 = s
\right] \\
&\ge H \Delta_\infty \\
...
2.  the **natural policy gradient** (NPG),
    a first-order approximation of TRPO;
3.  **proximal policy optimization** (PPO-penalty),
    a "soft relaxation" of TRPO;
4.  the **clipped surrogate objective** (PPO-clip),
...
\right] \\
&= \E_{\tau \sim \rho^{\picurr}} \left[
    \sum_{\step=0}^{\Step-1} \log \picurr(a_\step \mid s_\step) - \log \pinext(a_\step \mid s_\step)
\right] \\
\end{aligned}
$$ {#eq-kld-trajectories}
...
\E_{\tau \sim \rho^{\picurr}} \left[
    \sum_{\step=0}^{\Step-1}
    \E_{a'_\step \sim \pi^{\thetanext}(\cdot \mid s_\step)}
    A^{\picurr}(s_\step, a'_\step)
\right] \\
& \text{where } \kl{\rho^{\picurr}}{\rho^{\pinext}} < \delta
\end{aligned}
$$ {#eq-trpo}
...
The second-order approximation to $d^\iter(\theta) := \kl{p_\thetacurr}{p_\theta}$
is given by
...
1.  Write down the Taylor expansion of $d^\iter(\theta)$ around $\thetacurr$.
1.  Show that the zeroth-order term $d^\iter(\thetacurr)$ is zero.
1.  Show that the gradient $\nabla d^\iter(\theta) \vert_{\theta = \thetacurr}$ is zero.
1.  Show that the Hessian $\nabla^2 d^\iter(\theta) \vert_{\theta = \thetacurr}$ equals $F_\thetacurr$.
:::
...
and the second expression shows that it is the expected Hessian matrix of the negative-log-likelihood of the underlying distribution.
It can also be shown that it is precisely the Hessian of the KL divergence (with respect to either one of the parameters).
...
(The zeroth and first order terms are both zero in our case due to properties of the KL divergence.)
:::
...
and approximate the expectation by a sample mean,
since we can write the Fisher information matrix as

$$
F_{\theta} = \E_{\tau \sim \rho^{\pi_\theta}} \left[ \sum_{\step=0}^{\Step-1} (\nabla \log \pi_\theta (a_\step \mid s_\step)) (\nabla \log \pi_\theta(a_\step \mid s_\step))^\top \right].
...
over Bernoulli distributions $p_\theta \in \triangle(\{ 0, 1 \})$.
...
```{python}
#| fig-cap: The space of Bernoulli distributions.
#| fig-align: center
...
$$
\begin{aligned}
\theta^{k+1} &\gets \arg\max_{\thetanext} \E_{\tau \sim \rho^{\picurr}} \left[ \sum_{\step=0}^{\Step-1} \E_{a'_\step \sim \pi_{\theta}(s_\step)} A^{\picurr}(s_\step, a'_\step) \right] - \lambda \kl{\rho^{\picurr}}{\rho^{\pinext}}
\end{aligned}
$$ {#eq-ppo-penalty}
...
\begin{aligned}
    \kl{\rho^{\picurr}}{\rho^{\pi_{\theta}}} & = \E_{\tau \sim \rho^{\picurr}} \left[\log \frac{\rho^{\picurr}(\tau)}{\rho^{\pi_{\theta}}(\tau)}\right]                                                       \\
                                           & = \E_{\tau \sim \rho^{\picurr}} \left[ \sum_{\step=0}^{\Step-1} \log \frac{\picurr(a_\step \mid s_\step)}{\pi_{\theta}(a_\step \mid s_\step)}\right] \\
                                           & = \E_{\tau \sim \rho^{\picurr}} \left[ \sum_{\step=0}^{\Step-1} \log \frac{1}{\pi_{\theta}(a_\step \mid s_\step)}\right] + c
\end{aligned}
$$
...
L^k(\theta)
=
\E_{s_0, \dots, s_{\Step-1} \sim \rho^{\picurr}} \left[
    \sum_{\step=0}^{\Step-1} \E_{a_\step \sim \pi_{\theta}(s_\step)} A^{\picurr}(s_\step, a_\step)
\right] - \lambda \E_{\tau \sim \rho^{\picurr}}
\left[ \sum_{\step=0}^{\Step-1} \log \frac{1}{\pi_{\theta}(a_\step \mid s_\step)}\right]
$$
...
$$
\E_{a_\step \sim \pinext(s_\step)} A^{\picurr}(s_\step, a_\step)
=
\E_{a_\step \sim \picurr(s_\step)} \left[
    \frac{\pinext(a_\step \mid s_\step)}{\picurr(a_\step \mid s_\step)} A^{\picurr}(s_\step, a_\step)
\right]
$$ {#eq-likelihood-ratio}
...
Suppose $a^+$ is a "good" action for $\picurr$ in state $s_\step$,
i.e. $A^\picurr(s_\step, a^+) > 0$.
Then maximizing @eq-likelihood-ratio encourages $\thetanext$ to _increase_ the probability ratio
$\pinext(a_\step \mid s_\step) / \picurr(a_\step \mid s_\step)$.

Otherwise,
if $a^-$ is a "bad" action for $\picurr$ in state $s_\step$
(i.e. $A^\picurr(s_\step, a^-) < 0$),
...
L^k(\theta) = \E_{\tau \sim \rho^{\picurr}} \left[
    \sum_{\step=0}^{\Step-1} \left(
    \frac{\pi_\theta(a_\step \mid s_\step)}{\picurr(a_\step \mid s_\step)} A^{\picurr}(s_\step, a_\step)
    - \lambda \log \frac{1}{\pi_\theta(a_\step \mid s_\step)}
    \right)
\right]
$$
:::

Now we can estimate this function by a sample mean over trajectories from $\picurr$. Remember that to complete a single iteration of PPO, we execute
...
L^k(\thetanext) = \E_{\tau \sim \rho^{\picurr}} \left[
    \sum_{\step=0}^{\Step-1}
    \frac{\pi_\thetanext(a_\step \mid s_\step)}{\picurr(a_\step \mid s_\step)} A^{\picurr}(s_\step, a_\step)
\right]
\approx V(\pi_\thetanext) - V(\picurr).
$$ {#eq-penalty-no-kl}

In the following part,
define the policy ratio at time $\step \in [\Step]$ as

$$
\Lambda_\step(\thetanext, \thetacurr) = \frac{\pinext(a_\step \mid s_\step)}{\picurr(a_\step \mid s_\step)}.
$$ {#eq-policy-ratio}
...
and constrain $\Lambda_\step(\thetanext, \thetacurr) \in (1-\epsilon, 1+\epsilon)$.
Formally,

$$
\Lambda_\step^\text{clipped}(\thetanext, \thetacurr)
=
\text{clip}(\Lambda_\step(\thetanext, \thetacurr), 1-\epsilon, 1+\epsilon),
$$ {#eq-clip-ratio}
...
suppose $a_\step^+$ and $a_\step^-$ are a "good" and "bad" action in $s_\step$,
i.e. $A^\picurr(s_\step, a_\step^+) > 0$ and $A^\picurr(s_\step, a_\step^-) < 0$.
By clipping $\Lambda_\step(\thetanext, \thetacurr)$,
no additional benefit is gained by increasing $\pinext(a_\step^+ \mid s_\step)$
above $(1+\epsilon) \picurr(a_\step^+ \mid s_\step)$,
or by decreasing $\picurr(a_\step^- \mid s_\step)$
under $(1-\epsilon) \picurr(a_\step^- \mid s_\step)$.
:::

As a final step,
we only use this clipped objective if it is smaller than the original objective.
For example, if $A^\picurr(s_\step, a_\step) > 0$ and $\Lambda_\step(\thetanext, \thetacurr) \ll 1 - \epsilon$,
then the clipped objective would disproportionately incentivize taking action $a_\step$.
The same thing happens if $A^\picurr(s_\step, a_\step) < 0$ and $\Lambda_\step(\thetanext, \thetacurr) > 1+\epsilon$.
...
#| fig-subcap:
#| - For a "good" action $a_\step^+$ with positive advantage.
#| - For a "poor" action $a_\step^-$ with negative advantage.
#| layout-ncol: 2
...
$$
L^\iter(\theta) = \E_{\tau \sim \rho^{\picurr}} \left[
    \sum_{\step=0}^{\Step-1}
    \min \left\{
        \Lambda_\step(\theta, \thetacurr) A^{\picurr}(s_\step, a_\step),
        \text{clip}(
            \Lambda_\step(\theta, \thetacurr),
            1-\epsilon,
            1+\epsilon
        )
        A^\picurr(s_\step, a_\step)
    \right\}
\right]
$$ {#eq-clip-objective}
:::
...
## Bibliographic notes and further reading {#sec-pg-bib}

@sutton_policy_1999 popularized the term "policy gradient methods".
Policy gradient methods,
including REINFORCE [@williams_simple_1992],
were some of the earliest reinforcement learning methods
[@witten_adaptive_1977; @barto_neuronlike_1983; @sutton_temporal_1984].

@kakade_approximately_2002 introduced the performance difference lemma,
which was used to prove theoretical guarantees in several later works.

The expression of the policy gradient in terms of the Q-function
was simultaneously observed by @marbach_simulation-based_2001
and @sutton_policy_1999.

The natural gradient was suggested by @amari_natural_1998 for general optimization problems
and then applied to the policy gradient by @kakade_natural_2001.
The **Natural Actor-Critic** algorithm combines actor-critic learning
with the natural policy gradient estimator [@peters_natural_2005; @peters_natural_2008; @bhatnagar_natural_2009].

Natural gradient descent is effective
because it accounts for the _geometry_ induced by the parameterization
and thereby optimizes over the _distributions_ themselves.
Another optimization algorithm that accounts for parameter-space geometry
is **mirror descent** [@nemirovskij_problem_1983; @beck_mirror_2003].
At each optimization step,
this maps the parameter vector to a _dual space_
defined to capture the desired geometry.
It then takes a gradient descent step on the dual parameters
and maps the result back to an updated "primal" parameter vector.
One can also apply mirror descent to policy gradient algorithms [@mahadevan_sparse_2012; @mahadevan_basis_2013].

A key reason for the popularity of policy gradient methods
is that they are easily _parallelizable_ across multiple computing threads.
Each thread collects rollouts and computes its own gradient estimates,
and every once in a while,
these are accumulated across threads and used to perform an update of the parameters,
which are then synchronized across the different threads.
@mnih_asynchronous_2016 present multi-threaded variants of policy gradient methods
as well as several algorithms from @sec-fit.
@schulman_trust_2015 proposed TRPO (@def-trpo-update).
@schulman_high-dimensional_2016 introduced
**generalized advantage estimation** (GAE),
which accepts some bias in the gradient estimate
in exchange for lower variance.
This tradeoff is controlled by the parameter $\lambda$,
the same parameter as in $\text{TD}(\lambda)$.
@schulman_proximal_2017 introduced the PPO-KL and PPO-Clip algorithms.
@wu_scalable_2017 introduced the ACKTR algorithm,
which seeks to scale NPG up to deep learning models
where naively approximating the Fisher information matrix
is impractical.
It does so using the Kronecker-factored approximation proposed in @martens_optimizing_2015.
@shao_deepseekmath_2024 introduce a variant of PPO known as **Group Relative Policy Optimization**\index{group relative policy optimization} (GRPO).

Policy gradient methods, being on-policy algorithms,
inherently have worse sample efficiency than off-policy algorithms,
since naively each sample can only be used to update at the value of the parameter vector
that it was sampled at.
@wang_sample_2017 introduce the **actor-critic with experience replay** (ACER) algorithm,
which builds on the **Retrace** Q function estimator [@munos_safe_2016]
to estimate policy gradients using previously collected experience [@lin_self-improving_1992].

The **soft Q-learning** and **soft actor-critic** algorithms introduced in @haarnoja_reinforcement_2017 and @haarnoja_soft_2018 respectively
add the **entropy** of the policy
to the objective function.
This practice is known as **maximum entropy reinforcement learning**
since we seek to maximize the policy entropy
alongside the expected total reward.
This encourages policies that "act more randomly",
which aids exploration.
...
I kept the overall structure of this chapter the same as it was presented in class,
which has remained relatively consistent throughout the years:
we begin by reviewing gradient descent,
followed by the REINFORCE algorithm,
and then more recent trust region-based methods.

I build up to REINFORCE with baselines
a little more explicitly than was done in the course,
by framing the statement of unbiasedness
for the different estimators
as explicit "theorems".
The proofs of these "theorems" have been left to a homework exercise.
This emphasizes that the fundamental framework for policy gradient methods,
i.e. weighting the gradient-log-probability of actions by some measure of the action's quality,
remains the same.

In constructing the bibliographic notes (@sec-pg-bib),
I referred to the references in @achiam_spinning_2018
and the citations in @sutton_reinforcement_2018 [ch. 13],
including recent developments such as GRPO.
:::
