...
```{python}
from utils import Float, Array, Callable, plt
from torchvision.datasets import MNIST
from config import MNIST_PATH
```

**Supervised learning** (SL) is another core subfield of machine learning,
alongside RL and unsupervised learning.
The term "supervised" refers to the type of data SL algorithms deal with:
...
In other words, the main concern of SL is **function approximation**
(also known as **modeling**):
given a dataset of inputs to a function,
paired with their corresponding outputs,
can we reconstruct or approximate the unknown function?
...
such as the state transitions or the reward function,
any of which might be unknown.
Instead, we could interact with the environment,
collecting a dataset of input-output pairs of these functions,
and then plug in an SL algorithm to approximate the unknown functions.

The rest of this chapter will cover details of SL,
that is, how to use a dataset of labelled samples $(x_1, y_1), \dots, (x_N, y_N)$
to find a function $f$ that approximates the true function.
...
::: {#def-parameterized-empirical-risk-minimization}
