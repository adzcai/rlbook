...
- In a finite-horizon MDP (@sec-finite-horizon-mdps),
  we can apply dynamic programming,
  working backwards from the end of the time horizon, to compute the optimal value function exactly.
- In an infinite-horizon MDP (@sec-infinite-horizon-mdps),
  we can use @sec-value-iteration,
  which iterates the Bellman optimality operator @eq-bellman-optimality-operator
  to approximately compute the optimal value function.
