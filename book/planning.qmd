...
Then, we can replace the simulation step of @def-mcts-algorithm with evaluating $r = v(s_\text{next})$, where $s_\text{next} = P(s_\text{new}, a_\text{new})$.
...
we could use @sec-imitation_learning (that is, imitation learning)
...
Recall the @sec-policy_iteration algorithm from the @sec-mdps.
...
Now, we can use @sec-imitation_learning
...
- **Policy improvement**: Let $\pi^t_\text{MCTS}$ denote the policy obtained by @def-mcts-policy-value with $\pi_{\theta^t}$ and $v_{\lambda^t}$. We use $\pi^t_\text{MCTS}$ to play against itself $M$ times. This generates $M$ trajectories $\tau_0, \dots, \tau_{M-1}$.
...
@sec-alpha-beta-search does this by _pruning_ away states that we already know to be suboptimal,
and @sec-mcts _approximates_ the value of states instead of evaluating them exactly.
