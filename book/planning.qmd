...
![The first two layers of the complete game tree of tic-tac-toe.](shared/tic_tac_toe.png)
...
## Alpha-beta search {#sec-alpha-beta-search}
...
Let us use the same simple game from @exm-min-max-example.
...
We've encountered this problem before in both the @sec-bandits setting and the @sec-mdps setting.
...
choosing the best action $a$ can be framed as a @sec-bandits problem,
...
The most commonly used bandit algorithm in practice for MCTS is the @sec-ucb algorithm.
...
Then, we can replace the simulation step of @def-mcts-algorithm with evaluating $r = v(s-\text{next})$, where $s-\text{next} = P(s-\text{new}, a-\text{new})$.
...
Recall the @sec-policy-iteration algorithm from the @sec-mdps.
...
Now, we can use @sec-imitation-learning
...
- **Policy improvement**: Let $\pi^t_\text{MCTS}$ denote the policy obtained by @def-mcts-policy-value with $\pi-{\theta^t}$ and $v-{\lambda^t}$. We use $\pi^t-\text{MCTS}$ to play against itself $M$ times. This generates $M$ trajectories $\tau-0, \dots, \tau-{M-1}$.
