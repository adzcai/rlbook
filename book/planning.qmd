...
We will see an explicit algorithm for this in @min-max-search.
...
## Monte Carlo Tree Search {#sec-monte-carlo-tree-search}
...
The most commonly used bandit algorithm in practice for MCTS is the @ucb algorithm.
...
Then, we can replace the simulation step of @mcts-algorithm with evaluating $r = v(s_\text{next})$, where $s_\text{next} = P(s_\text{new}, a_\text{new})$.
...
Then, plugging these into @mcts-policy-value
...
Recall the @policy_iteration algorithm from the @sec-mdps.
...
@alpha-beta-search does this by _pruning_ away states that we already know to be suboptimal,
and @monte-carlo-tree-search _approximates_ the value of states instead of evaluating them exactly.
