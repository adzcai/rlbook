...
Let $V_\hi^\star(s)$ denote the game score
under optimal play from both players
starting in state $s$ at time $\hi$.
...
to generate a policy $\pi_\text{guide}$ via behavior cloning
...
- **Policy evaluation**: Use behavior cloning to find a set of policy parameters $\theta^{t+1}$ that mimic the behavior of $\pi^t_\text{MCTS}$ and a set of value function parameters $\lambda^{t+1}$ that approximate its value function. That is,
  $$
  \begin{aligned}
