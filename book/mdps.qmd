...
::: {layout-ncol="2"}
![A deterministic policy](shared/deterministic_policy.png)

![A stochastic policy](shared/stochastic_policy.png)
:::
...
Here are some possible policies for the tidying MDP @exm-tidy-mdp:
...
::: {#def-autoregressive-trajectories}
...
::: {#thm-bellman-consistency-action}
...
The Bellman operator also gives us a concise way to express @thm-bellman-consistency for the value function:
...
## Solving finite-horizon MDPs {#sec-finite-horizon-mdps}
...
Note that this also gives simplified forms of the @thm-bellman-consistency for the optimal policy:
...
`dp_eval_finite` in @sec-eval-dp, but instead of *averaging* over the
...
::: {attention}
Which of the policies in @exm-tidy-policy are stationary?
:::
...
into the Bellman consistency equation @thm-bellman-consistency to account for the discounting:
...
infinite-horizon case. As in @def-optimal-policy-finite, an **optimal policy** $\pi^\star$
...
Although PI appears more complex than VI, we'll use @thm-bellman-contraction to show convergence. This will give us the same runtime bound as value iteration and iterative policy evaluation for an $\epsilon$-optimal value function @rem-iterations-vi, although in practice, PI often converges much faster.
