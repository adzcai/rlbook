...
Here are some possible policies for the tidying MDP @exm-tidy_mdp:
...
Could any of the policies in @exm-tidy_policy have generated this trajectory?
::::

Note that for a state-dependent policy, using the Markov property @def-markov,
...
::: {#def-autoregressive_trajectories}
...
::: {#thm-bellman_consistency}

#### Bellman consistency equation for the value function
...
::: {#thm-bellman_consistency_action}

#### Bellman consistency equation for action-values
...
The Bellman operator also gives us a concise way to express @thm-bellman_consistency for the value function:
...
## Solving finite-horizon MDPs {#sec-finite_horizon_mdps}
### Policy evaluation in finite-horizon MDPs {#sec-eval_dp}
...
#### It is optimal to be greedy with respect to the optimal value function
...
Note that this also gives simplified forms of the @thm-bellman_consistency for the optimal policy:

:::: {#thm-bellman_consistency_optimal}
...
`dp_eval_finite` in @sec-eval_dp, but instead of *averaging* over the
...
## Infinite-horizon MDPs {#sec-infinite_horizon_mdps}
...
Code-wise, we can reuse the `MDP` class from before @def-finite_horizon_mdp and set `mdp.H = float('inf')`.
...
:::{attention}
Which of the policies in @exm-tidy_policy are stationary?
:::
...
into the Bellman consistency equation @thm-bellman_consistency to account for the discounting:
...
\end{aligned}
$$ {#eq-bellman_consistency_infinite}
...
Recall from @def-bellman_operator that the Bellman operator $\mathcal{J}^{\pi}$
...
:::{prf:proof} Proof of @thm-bellman_contraction
...
The backwards DP technique we used in the finite-horizon case (@sec-eval_dp) no
...
infinite-horizon case. As in @def-optimal_policy_finite, an **optimal policy** $\pi^\star$
...
:::{attention}
Verify this by modifying the proof @thm-optimal_greedy from the finite-horizon case.
:::

So how can we compute such an optimal policy? We can't use the backwards
DP approach from the finite-horizon case @def-pi_star_dp since there's no "final timestep" to start
...
$$ {#eq-bellman_optimality_operator}
...
#### Value iteration {#sec-value_iteration}
...
is exactly the same as @sec-iterative_pe! This is because value iteration is simply
...
This means, using @rem-iterations_vi, we need to run value iteration for
...
#### Policy iteration {#sec-policy_iteration}
...
Although PI appears more complex than VI, we'll use @thm-bellman_contraction to show convergence. This will give us the same runtime bound as value iteration and iterative policy evaluation for an $\epsilon$-optimal value function @rem-iterations_vi, although in practice, PI often converges much faster.

:::: {#thm-pi_iter_analysis}
