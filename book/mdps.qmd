...
Similarly,
a history-independent policy $\pi$'s **action-value function** (aka Q function)
...
Can we mitigate this "greedy worsening"? What if instead of approximating the optimal value function and then acting greedily by it at the very end, we iteratively improve the policy and value function *together*? This is the idea behind **policy iteration**.
In each step, we simply set the policy to act greedily with respect to its own value function.
...
The number of iterations required for an
...
## Bibliographic notes and further reading {#sec-mdps-bib}
...
MDPs are the most common framework used throughout RL.
See @sec-intro-bib for a list of popular textbooks on RL.
...
We also clarify "Bellman _consistency_ equations" and "Bellman _optimality_ equations" throughout.

I also elaborate more on why policy iteration converges more quickly
than value iteration.
This was one of the most frequent questions from students.
...
thus I distinguish between episodic and continuing tasks earlier on in the chapter.
:::
