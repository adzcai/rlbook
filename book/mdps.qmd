...
The proof is left as @exr-proof-bellman-consistency.
...
The Bellman consistency equations (@eq-bellman-consistency)
...
We'll investigate optimality only for _state-dependent_ policies.
In @thm-optimal-greedy,
we'll see that this is actually sufficient:
the best state-dependent policy performs
at least as well as
...
Let $\pi^\star$ be a state-dependent policy.
We say that $\pi^\star$ is **optimal**
if $V_\hi^{\pi^\star}(s) \ge V_\hi^{\pi}(s)$
for any state-dependent policy $\pi$ at any time $\hi \in [\hor]$.
In other words,
an optimal state-dependent policy achieves the highest possible
...
As in the Bellman consistency equations (@thm-bellman-consistency),
...
Like the Bellman consistency equations (@eq-bellman-consistency),
This is a system of $H |\mathcal{S}|$ equations
in $H |\mathcal{S}|$ unknowns,
...
We now construct a deterministic state-dependent optimal policy
by acting *greedily*
...
we use these to compute the optimal values at time $\hor-2$ using the Bellman optimality equations (@eq-bellman-optimality),
...
1. $\pi$ is optimal across state-dependent policies (@def-optimal-policy-finite).
2. $\pi$ is optimal across all policies (@eq-optimal-all).
3. $V^\pi = V^\star$.
4. $Q^\pi = Q^\star$.
5. $V^\pi$ satisfies the Bellman optimality equations (@eq-bellman-optimality).
...
3 and 5 are the same by the Bellman optimality equations (@thm-bellman-optimality).
