...
This chapter will introduce the most popular mathematical formalization for such tasks:
...
## Introduction to Markov decision processes {#sec-finite-horizon-mdps}
...
That is, the agent takes up to $\hor$ actions,
where $\hor$ is some finite positive integer.
...
Steps 2 and 3 are repeated $\hor$ times,
resulting in a sequence of states and actions $s_0, a_0, \dots, s_{\hor-1}, a_{\hor-1}, s_\hor$,
where $s_\hor$ is some **terminal state**.
...
$$
\mathcal{M} = (\mathcal{S}, \mathcal{A}, P_0, P, r, \hor).
...
-   Markov decision processes (MDPs) are a framework for sequential
    ...
    $P_0 \in \Delta(\mathcal{S})$, a transition function $P(s' \mid s, a)$, and a
