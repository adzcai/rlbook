...
3.  An **initial state distribution** $\mu \in \triangle(\mathcal{S})$.
...
Suppose actions are chosen according to a state-dependent policy.
...
%%| fig-cap: A trajectory is generated by taking a sequence of actions according to the state-dependent policy.
...
whose distribution depends on the policy's trajectory distribution $\rho^\pi$ (@eq-autoregressive-trajectories).
...
Let $\pi$ be a state-dependent policy.
...
Similarly,
a state-dependent policy's **action-value function** (aka Q function)
...
#### Bellman consistency equations {#sec-bellman-consistency}
...
#### Proof by relating the value function and action-value function

Use the law of iterated expectations
to show that the value is the expected action-value over
...
to show that the action-value is the sum of
...
Putting these together reveals the Bellman consistency equations (@eq-bellman-consistency).
...
hold for the value functions of state-dependent policies.
...
and evaluate the next state using $v$ (@fig-updated-bellman).
...
#### Dynamic programming {#sec-finite-eval-dp}
...
We call the policy $\pi^\star$ **optimal** at time $\hi$ if,
given any partial trajectory
$\tau_{\le \hi} = (s_0, a_0, r_0, \dots, s_{\hi-1}, a_{\hi-1}, r_{\hi-1}, s_{\hi})$,
it does at least as well as any other policy $\pi^\text{any}$
(which may depend on the history):

$$
\begin{gathered}
\forall \pi,
\quad
\E_{\tau \sim \rho^{\pi^\star}} \left[
    r_\hi + \cdots + r_{\hor-1} \mid \tau_{\le \hi}
\right] \\
\ge
\E_{\tau \sim \rho^{\pi}} \left[
    r_\hi + \cdots + r_{\hor-1} \mid \tau_{\le \hi}
\right].
\end{gathered}
$$ {#eq-optimal-policy-finite}

By conditioning on $\tau_{\le \hi}$,
we mean the event that the state, action, and observed reward at timestep $\hi'$
are $s_{\hi'}, a_{\hi'}, r_{\hi'}$ respectively for all $\hi' < \hi$
and $s_\hi$ is the state observed at timestep $\hi$.

If $\pi^\star$ is optimal at time $\hi = 0$,
we simply call it optimal.
(@exr-optimal-time asks you to show that optimality at time $\hi$
implies optimality at all later times.)
:::

::: {#exr-optimal-time}
#### Optimality at later times

Show that if $\pi^\star$ is optimal at time $\hi$,
then it is also optimal at time $\hi+1$.
One way is to show the contrapositive:
if $\pi^\star$ is _not_ optimal at time $\hi+1$,
it _cannot_ be optimal at time $\hi$.
That is,
if there exists another policy $\pi'$
and a partial trajectory $\tau_{\le \hi+1}$
such that

$$
\E_{\tau \sim \rho^{\pi^\star}} \left[
    r_{\hi+1} + \cdots + r_{\hor-1} \mid \tau_{\le \hi+1}
\right]
<
\E_{\tau \sim \rho^{\pi'}} \left[
    r_{\hi+1} + \cdots + r_{\hor-1} \mid \tau_{\le \hi+1}
...
there must then exist a policy $\pi$
and a partial trajectory $\tau_{\hi}$
such that
...
In fact, _every_ finite-horizon MDP
has an optimal policy that is **state-dependent** and **deterministic**.
This is a striking result
that greatly narrows down the space of policies
that we have to search through.
We will prove it by induction in @thm-optimal-greedy
and construct such an optimal policy along the way using **dynamic programming**.
...
don't benefit over simply state-dependent ones in MDPs.
:::

Now that we know (and will prove) that a _state-dependent_ optimal policy exists,
it makes sense to define the _optimal value function_:
...
achievable by any state-dependent policy $\pi$:
...
We now construct a state-dependent, deterministic optimal policy
by acting *greedily*
...
The greedy policy $\pi_\hi^{Q^\star}(s)$, where $Q^\star$ is the optimal action-value function (@eq-optimal-action-value), is an optimal policy (@def-optimal-policy-finite). We will prove this in @def-pi-star-dp.
:::

$\pi^{Q^\star}$ is a policy just like any other.
In particular,
we can write down its value and action-value functions.

::: {#exr-optimal-policy-value}
#### Value functions of greedy optimal policy

Show that the value and action-value function of $\pi^{Q^\star}$
are $V^\star$ and $Q^\star$ respectively (@def-optimal-value).
That is,
...
The optimal value function and action-value function (@def-optimal-value) satisfy,
for all timesteps $\hi \in [\hor]$,

$$
\begin{aligned}
    \forall s \in \mathcal{S},
    \quad V_\hi^\star(s) &= \max_a Q_\hi^\star(s, a) \\
    \forall s \in \mathcal{S},
    a \in \mathcal{A},
    \quad
...
This can be derived by applying
the Bellman consistency equations (@thm-bellman-consistency)
to $\pi^{Q^\star}$.

Furthermore, this system of equations uniquely identifies $V^\star$ and $Q^\star$:
they are the _only_ functions that satisfy this system.
That is, if a sequence of functions
$v_h : \mathcal{S} \to \R$ and $q_h : \mathcal{S} \times \mathcal{A} \to \R$
for $\hi \in [\hor]$
satisfies @eq-bellman-consistency-optimal,
then we know $v = V^\star$ and $q = Q^\star$.

Note that these equations don't need the letter $\pi$ at all,
although they can be thought of
as the Bellman consistency equations for any optimal policy.
This will prove useful when we discuss **off-policy algorithms**
later in the course.
:::

::: {#exr-prove-bellman-optimality}
#### Proof of uniqueness

Prove that $V^\star$ and $Q^\star$
are the _unique_ solution to @eq-bellman-consistency-optimal.
(Try a proof by contradiction.)
:::

How can we compute $\pi^{Q^\star}$?
We can compute it using **dynamic programming** (DP).
In doing so,
we will also prove its optimality by induction.
Essentially,
we begin at the _end_ of the trajectory,
where the optimal action is simply the one that yields highest immediate reward.
Once the optimal values at timestep $\hor-1$ are known,
we use these to compute the optimal values at $\hor-2$,
and proceed backwards through the trajectory.
...
::: {#def-pi-star-dp}
#### Dynamic programming to solve a finite-horizon MDP

The inductive hypothesis is that $\pi^{Q^\star}$
is optimal at time $\hi$.
...
so the $Q$-function is simply the immediate reward:
...
there is only one action to maximize over.
...
**Recursion.**
Then, we can work backwards in time,
...
suppose $\pi^\star$ is optimal at time $\hi+1$.
Then for any other policy $\pi$,
and for any partial trajectory $\tau_{\le \hi}$,
...
={}&
\E_{\substack{a \sim \pi_\hi(s_\hi) \\ s_{\hi+1} \sim P(s_\hi, a)}} \left[
    r_\hi + \E_{\tau \sim \rho^\pi} [
...
and computing the optimal value function (@sec-value-iteration).
Finally, we'll present the policy iteration algorithm (@sec-policy-iteration).
...
::: {#thm-infinite-bellman-consistency}
...
$$ {#eq-infinite-bellman-consistency}
...
The Bellman consistency equations (@thm-infinite-bellman-consistency)
...
The Bellman consistency equations (@thm-infinite-bellman-consistency)
yield a system of equations we can solve
...
### Optimality {#sec-infinite-optimality}
...
DP approach from the finite-horizon case @def-pi-star-dp
...
As before,
thinking of the r.h.s. of @eq-bellman-optimality as an operator on value functions
