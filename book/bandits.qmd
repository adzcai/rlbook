...
This is the **Bernoulli bandits** problem,
which we'll use throughout the chapter.

:::: {layout-ncol="2"}
::: {#def-bernoulli-bandit}
#### Bernoulli bandits problem\index{Bernoulli bandits}
...
![Each arm $\arm$ in a Bernoulli bandits is a biased coin with probability $\mu^\arm$ of showing heads. Each "pull" corresponds to flipping a coin. The goal is to maximize the number of heads given $\Ep$ flips. Image from @pixabay_coins_2016.](shared/flipping-coins.jpg)
::::
...
Consider a two-armed Bernoulli bandits with $\mu^0 = 0$ and $\mu^1 = 1$.
...
We've been working in the Bernoulli bandits setting,
...
In the medical trial example (@exm-clinical-trial),
...
Rather than the Bernoulli bandits problem (@def-bernoulli-bandit),
...
for the Bernoulli bandits problem:
...
For instance, in Bernoulli bandits,
...
Note that $\widehat \Sigma_\ep^\arm$ might not be invertible.
...
I also focus exclusively on Bernoulli bandits for the majority of the chapter
(up until contextual bandits).
