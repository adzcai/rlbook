...
| ------------------------------------------------- | ---------------------------------- | ---------------------------------- |
| stateless/invariant ($\vert\mathcal{S}\vert = 1$) | (trivial)                          | multi-armed bandits                |
...
::: {#rem-mab-mdp-connection}
#### Connection between MAB and MDP settings

A multi-armed bandit is a special case of a finite-horizon Markov decision process
with a single state ($|\mathcal{S}| = 1$) and a horizon of $\Step = 1$.
(Conceptually speaking, the environment is "stateless",
but an MDP with "no states" doesn't make sense.)
As such,
Each arm is an action: $|\mathcal{A}| = \Arm$.
the expected value of pulling arm $\arm$,
which we denote $\mu^\arm$ in this chapter,
is exactly the Q-function (@def-q):

$$
\mu^\arm = Q(\arm)
$$ {#eq-q-mean}

Note that we omit the arguments for $\pi, \step$, and $s$,
which are meaningless in the MAB setting.
:::
...
and then use that knowledge to make the best possible decision at the last step.
...
There is also a **Uniform-PAC** bound.
We won't discuss these in detail here;
see the bibliographic notes for more details.
:::
...
(See @sec-bg-o-notation if you're unfamiliar with big-oh notation.)
...
for $N$ i.i.d. samples $\widetilde r_0, \dots, \widetilde r_{N-1}$ from arm $\arm$,

$$
| \widetilde \mu^\arm_N - \mu^\arm | \le \sqrt{
    \frac{\ln( 2 / \delta )}{2 N}
}
$$ {#eq-hoeffding-ucb}

with probability at least $1 - \delta$,
where $\widetilde \mu^\arm_N = \frac{1}{N} \sum_{n=0}^{N-1} \widetilde r_n$.
...
$$
| \widetilde \mu^\arm_{N^\arm_\ep} - \mu^\arm | \le \sqrt{
    \frac{\ln( 2 \ep / \delta' )}{N}
}
$$ {#eq-ucb-specific-arm}

with probability at least $1 - \delta'$ as well.
But notice that $\widetilde \mu^\arm_{N^\arm_\ep}$ is identically distributed to $\hat \mu^\arm_\ep$:
...
William Thompson studied the two-armed mean distinction problem
in @thompson_likelihood_1933 and @thompson_theory_1935.
...
@berry_bandit_1985 is a comprehensive treatment of bandit problems from a statistical perspective.
