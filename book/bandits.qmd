...
We’ll label them $0, \dots, K-1$ and use *superscripts* to indicate the arm index;
since we seldom need to raise a number to a power,
this won’t cause much confusion.
In this chapter,
we’ll consider the **Bernoulli bandit** setting from the examples above,
where arm $k$ either returns reward $1$ with probability $\mu^k$ or $0$ otherwise.
The agent gets to pull an arm $T$ times in total.
We can formalize the Bernoulli bandit in the following Python code:
...
The proof of this inequality is beyond the scope of this book. See @vershynin_high-dimensional_2018 Chapter 2.2.
...
It turns out that setting $\epsilon_t = \sqrt[3]{K \ln(t)/t}$ also achieves a regret of $\widetilde O(t^{2/3} K^{1/3})$ (ignoring the logarithmic factors). (We will not prove this here; a proof can be found in [@agarwal_reinforcement_2022].)
...
In @sec-etc-regret-analysis, we were able to compute this bound
...
arm*. In particular, we’ll define $\widetilde r^k_n$ to be the $n$th sample
from arm $k$, and $\widetilde \mu^k_n$ to be the sample average of the first
...
to get a bound on $\widetilde \mu^k_n$.

So how can we extend our bound on $\widetilde\mu^k_n$ to $\hat \mu^k_t$?
...
$$
\begin{aligned}
    \pr\left( \forall n \le t, |\widetilde \mu^k_n - \mu^k | \le \sqrt{\frac{\ln(2/\delta)}{2n}} \right) &\ge 1-t\delta.
\end{aligned}
$$ {#eq-bandits-uniform-count}

In particular, since $N^k_t \le t$, and $\widetilde \mu^k_{N^k_t} = \hat \mu^k_t$ by definition, we have
...
and show $\text{Regret}_T = \widetilde O(\sqrt{TK})$.
...
following sense. @lai_asymptotically_1985 prove an
...
achieves a regret bound of $\widetilde{O}(\sqrt{TK})$. So in this problem,
we would achieve regret $\widetilde{O}(\sqrt{TK|\mathcal{X}|})$ in the
...
follows from some algebra on the definition of the OLS estimator @eq-ols-bandit.
:::
...
Using similar tools for UCB, we can also prove an $\widetilde{O}(\sqrt{T})$
regret bound. The full details of the analysis can be found in Section 3 of [@agarwal_reinforcement_2022].
