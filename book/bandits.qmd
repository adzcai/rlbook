...
by pulling each arm $N_\text{explore}> 1$ times before committing.
...
$$
a_\ep := \begin{cases}
\ep \remainder \Arm & \ep < N_\text{explore} \Arm \\
\arg\max_{\arm \in [\Arm]} \hat \mu^\arm & \ep \ge N_\text{explore} \Arm,
\end{cases}
$$ {#eq-etc}

where $\ep \remainder \Arm$ indicates the remainder of $\ep$ when divided by $\Arm$,
and

$$
\hat \mu^\arm := \frac{1}{N_\text{explore}} \sum_{n=0}^{N_\text{explore}} r_{n N_\text{explore} + \arm}
$$ {#eq-etc-average}

denotes the empirical mean reward of arm $\arm$.
:::
...
and let $\hat \mu^\arm_\ep$ denote the sample average of the corresponding rewards:
...
Note that we let the exploration probability $\epsilon_\ep$ vary over time.
...
In particular, after $\ep$ overall pulls,
we'd like to compute some upper confidence bound $M^\arm_\ep$ for each arm $\arm$
such that $\mu^\arm_\ep \le M^\arm_\ep$ with high probability,
...
let $N^\arm_\ep$ denote the number of times
we pull arm $\arm$ within the first $\ep$ overall pulls,
and let $\hat \mu^\arm_\ep$ denote the sample average of the corresponding rewards.
...
::: {.proof}
In the proof of the ETC regret bound (@thm-etc-regret),
we used Hoeffding's inequality (@thm-hoeffding) to compute a CI for $\mu^\arm$
given $N_\text{explore}$ samples from arm $\arm$.
Hoeffding's inequality is useful for computing CIs of the mean of a bounded random variable
given a fixed number of i.i.d. samples.

However,
we can't directly apply Hoeffding's inequality to $\hat \mu^\arm_\ep$,
since $\hat \mu^\arm_\ep$ is the sample mean of a _random_ number $N^\arm_\ep$ of samples:
the number of times we pull each arm depends on the agent's actions,
which in turn depend on the random rewards.
We must be careful to _condition_ on $N^\arm_\ep$
before applying Hoeffding's inequality.
That is,
_conditioned_ on $N^\arm_\ep$,
the $N^\arm_\ep$ pulls of arm $\arm$ are i.i.d.,
in which case Hoeffding's inequality gives the bound

$$
\pr\left(| \hat \mu^\arm_\ep - \mu^\arm | > \sqrt{
    \frac{\ln(2/\delta)}{2N^\arm_\ep}
} \right) \le \delta.
$$ {#eq-hoeffding-etc}

But we need a bound that holds _unconditionally_ of $N^\arm_\ep$.
Well, we know $N^\arm_\ep \le \ep$
(where equality would be the case if and only if we pulled arm $\arm$ every time),
so we can use the **union bound** to compute a *looser* bound
that holds *uniformly* across all possible values of $N^\arm_\ep$:

$$
\begin{aligned}
    \pr\left(
        \forall N \le \ep, |\tilde \mu^\arm_N - \mu^\arm |
        \le
        \sqrt{\frac{\ln(2/\delta)}{2 N}}
    \right) &\ge 1- \ep \delta.
\end{aligned}
$$ {#eq-bandits-uniform-count}

where $\tilde \mu^\arm_n$ denotes the sample average of $n$ i.i.d. pulls of arm $\arm$.
Concretely,

$$
\begin{aligned}
\tilde \mu^\arm_N &:= \frac{1}{N} \sum_{n=0}^{N-1} \tilde r^\arm_n \\
\text{where } \tilde r^\arm_0, \dots, \tilde r^\arm_{N-1}
&\sim \text{arm } \arm \text{ i.i.d.}
\end{aligned}
$$ {#eq-arm-iid-samples}
...
The upper confidence bound algorithm chooses the $\ep$th pull by taking

$$
\begin{aligned}
a_\ep &:= \arg\max_{\arm \in [\Arm]} M^\arm_\ep, \\
\text{where } M^\arm_\ep &:= \hat \mu^\arm_\ep + \sqrt{
    \frac{\ln(2 \ep/\delta')}{2 N^\arm_\ep}
}.
\end{aligned},
$$ {#eq-ucb-bound}
...
$$
\begin{aligned}
    \regret &\le 2 \Arm \sqrt{2 \Ep \ln(2 \Ep \Arm /\delta'')} && \text{with probability } 1-\delta'' \\
...
I decided to include some more discussion of how to interpret asymptotic regret bounds:
