...
-   $d_0 \in \triangle(\mathcal{S})$ denotes the initial state distribution.
-   We call the state transitions the **dynamics** of the system and denote them by $f$:
    $$
    \st_{\hi+1} = f_\hi(\st_\hi, \act_\hi, w_\hi),
    $$ {#eq-lqr-dynamics}
    where $w_\hi$ denotes noise drawn from the distribution $\nu_\hi \in \triangle(\mathbb{R}^{n_w})$.
    Note that the dynamics may vary at each timestep $\hi$.
-   Instead of maximizing the reward function, we seek to minimize the **cost function** $c_\hi: \mathcal{S} \times \mathcal{A} \to \mathbb{R}$.
    Often, the cost function describes *how far away* we are from a **target state-action pair** $(\st^\star, \act^\star)$.
-   The agent acts over a *finite time horizon* $\hor \in \mathbb{N}$.

Together, these constitute a description of the environment

$$
\mathcal{M} = (\mathcal{S}, \mathcal{A}, d_0, f, \nu, c, \hor).
...
\begin{aligned}
    \min_{\pi_0, \dots, \pi_{\hor-1} : \mathcal{S} \to \mathcal{A}} \quad & \E \left[
        \sum_{\hi=0}^{\hor-1} c_\hi(\st_\hi, \act_\hi)
    \right] \\
...
That is, if we sample initial states from $d_0$,
act according to $\pi$,
...
Let $\pi = \{ \pi_0, \dots, \pi_{\hor-1} \}$ be a deterministic, time-dependent policy.
We include states $\st_\hi$, actions $\act_\hi$, and noise terms $w_\hi$ in the trajectory.
Then the density of a trajectory $\tau = (\st_0, \act_0, w_0, \dots, w_{\hor-2}, \st_{\hor-1}, \act_{\hor-1})$
can be expressed as follows:

$$
\begin{aligned}
\rho^{\pi}(\tau)
&:= d_0(\st_0) \times \ind{\act_0 = \pi_0(\st_0)} \\
&\quad {} \times \nu(w_0) \times \ind{\st_1 = f(\st_0, \act_0, w_0)} \\
&\quad {} \times \cdots \\
&\quad {} \times \nu(w_{\hor-2})
    \times \ind{\st_{\hor-1} = f(\st_{\hor-2}, \act_{\hor-2}, w_{\hor-2})}
    \times \ind{\act_{\hor-1} = \pi_{\hor-1}(\st_{\hor-1})} \\
&= d_0(\st_0) \ind{\act_0 = \pi_0(\st_0)}
    \prod_{\hi=1}^{\hor-1} \nu(w_{\hi-1})
    \ind{\st_{\hi} = f(\st_{\hi-1}, \act_{\hi-1}, w_{\hi-1})}
    \ind{\act_{\hi} = \pi_{\hi}(\st_{\hi})}
\end{aligned}
...
we can define its value function $V^\pi_\hi : \mathcal{S} \to \mathbb{R}$ at time $\hi \in [\hor]$
as the **cost-to-go** incurred by that policy in expectation:

$$
V^\pi_\hi (\st) = \E_{\tau \sim \rho^\pi} \left[
    \sum_{\hi'=\hi}^{\hor-1} c(\st_{\hi'}, \act_{\hi'})
    \mid \st_\hi = \st
\right]
$$ {#eq-value-control}
:::
...
$$
Q^\pi_\hi (\st, \act) = \E_{\tau \sim \rho^\pi} \left[
    \sum_{\hi'=\hi}^{\hor-1} c(\st_{\hi'}, \act_{\hi'})
    \mid (\st_\hi, \act_\hi) = (\st, \act)
\right]
$$ {#eq-q-lqr}
:::
...
$$
x_{\hi+1} = f(\st_\hi, \act_\hi, w_\hi) = A \st_\hi + B \act_\hi + w_\hi
$$ {#eq-lqr-linear-dynamics}

for all $\hi \in [\hor-1]$,
where $w_\hi \sim \mathcal{N}(0, \sigma^2) I$.
...
$$
c(\st_\hi, \act_\hi) = \st_\hi^\top Q \st_\hi + \act_\hi^\top R \act_\hi
...
Recall that $w_\hi$ is a noise term that makes the dynamics random.
...
$$
\begin{aligned}
    V^\star_\hi(\st) &= \min_{\pi} V^\pi_\hi(\st) \\
    &= \min_{\pi} \E_{\tau \sim \rho^\pi} \left[
            \sum_{\hi'=\hi}^{\hor-1} \left( \st_{\hi'}^\top Q \st_{\hi'} + \act_{\hi'}^\top R \act_{\hi'} \right)
            \mid \st_\hi = \st
        \right] \\
\end{aligned}
...
$$
\begin{aligned}
    Q^\star_\hi(\st, \act) &= \min_{\pi} Q^\pi_\hi(\st, \act) \\
    &= \min_{\pi} \E \left[
        \sum_{\hi'=\hi}^{\hor-1}\left(  \st_{\hi'}^\top Q \st_{\hi'} + \act_{\hi'}^\top R \act_{\hi'} \right)
        \mid \st_\hi = \st, \act_\hi = \act
    \right] \\
\end{aligned}
$$ {#eq-optimal-q-lqr}
:::
...
for some symmetric, positive-definite $n_\st \times n_\st$ matrix $P_\hi$ and scalar
$p_\hi \in \mathbb{R}$.
:::
...
Here we provide the concrete $P_\hi, p_\hi$, and $K_\hi$
used to compute the quantities above.

::: {#def-riccati}
#### Riccati equation

It turns out that the matrices $P_0, \dots, P_{\hor-1}$ used to define $V^\star$
...
The scalars $p_\hi$ obey the recurrence relation

$$
p_\hi = \mathrm{Tr}(\sigma^2 P_{\hi+1}) + p_{\hi+1}
$$ {#eq-lqr-proof-scalar}

and the matrices $K_0, \dots, K_{\hor-1}$ for defining the policy satisfy

$$
K_\hi = (R + B^\top P_{\hi+1} B)^{-1} B^\top P_{\hi+1} A.
$$ {#eq-k-pi}

By setting $P_\hor = 0$ and $p_\hor = 0$,
...
state at time $\hi$ behaves when we act according to this optimal policy.
...
Walk through the proof in @sec-optimal-lqr to verify that we can simply add $\hi$ for the time-dependent case.
:::
...
$$
\begin{aligned}
        \min_{\pi_0, \dots, \pi_{\hor-1} : \mathcal{S} \to \mathcal{A}} \quad
        & \E_{\st_0 \sim d_0} \left[
            \sum_{\hi=0}^{\hor-1} c(\st_\hi, \act_\hi)
        \right] \\
