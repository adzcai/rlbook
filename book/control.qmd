...
As such, most control theory algorithms,
like the ones we will explore in this chapter,
are _planning methods_ that do not require learning from data.
:::
...
-   We call the state transitions the **dynamics** of the system and denote them by $f$:
    ...
    We allow the dynamics to vary at each timestep $\step$.
...
Since we use a _cost function_ $c$ instead of a _reward function_ $r$,
...
Show that replacing $Q$ and $R$ with $(Q + Q^\top) / 2$ and $(R + R^\top) / 2$
...
Let's first frame this as a continuous-time dynamical system,
which is often more natural for physical phenomena,
and then discretize it to solve it with the discrete-time LQR.

From elementary physics,
the ball's velocity $\dot p(t)$ is the instantaneous change in its position,
and its acceleration $\ddot p(t)$ is the instantaneous change in its velocity.
Suppose we can apply a force $\act(t)$ to accelerate the ball.
This dynamical system is known as the **double integrator**.
Our goal is to move the ball so that it is stationary at position $p^\star = 0$.

How would we frame this as an LQR problem?
The effect of the control on the ball's position is nonlinear.
However, if we define our state as including both the position and velocity,
i.e.
...
\end{pmatrix}
\act(t).
$$ {#eq-double-integrator-system}

There are many ways to turn this into a discrete-time problem.
The simplest way is the (forward) _Euler method_,
where we choose a small step size $\Delta t$,
and explicitly use the limit definition of the derivative to obtain

$$
\st(t + \Delta t) \approx \st(t) + \Delta t \cdot \dot \st(t).
$$ {#eq-euler-discretization}

Applying this to @eq-double-integrator-system gives the system

$$
\st_{\step+1} = \begin{pmatrix}
1 & \Delta t \\
0 & 1
\end{pmatrix}
\st_\step
...
\end{pmatrix}
\act_\step,
$$ {#eq-discrete-double-integrator-system}

or explicitly in terms of the position and velocity,

$$
\begin{aligned}
p_{\step+1} &= p_\step + \Delta t \cdot \dot p_\step, \\
\dot p_{\step + 1} &= \dot p_\step + \Delta t \cdot \act_\step.
\end{aligned}
$$ {#eq-discrete-double-integrator-system-explicit}

```{python}
#| label: fig-lqr-accelerate
#| layout-ncol: 2
#| fig-cap: Visualization of a double integrator LQR system.
#| fig-subcap:
#| - The state and control input over time.
#| - The trajectory of $\st_\step$ plotted in the phase space.
...
We see that the LQR control
smoothly controls the position of the ball.
:::
...
::: {#def-lqr-solution}
#### Dynamic programming in LQR

Let $A, B, Q, R$ be the matrices that define the LQR problem,
and let $\sigma^2$ be the variance of the noise in the dynamics (@def-lqr).

1.  Initialize $P_\Step = 0, p_\Step = 0$.
2.  For each $\step = \Step - 1, \dots, 0$:
    1.  Compute $P_\step$ using the Riccati equation (@def-riccati) and $p_\step$ using @eq-lqr-scalar.
    2.  Compute $K_\step$ using @eq-k-pi.
    3.  These define the functions $V^\star_\step$ and $\pi^\star_\step$ by @thm-optimal-value-lqr-quadratic and @thm-optimal-policy-lqr-linear.
:::
...
How can we compute the expected state at time $\step$
when acting according to the optimal policy?
Let's first express $\st_\step$ in a cleaner way in terms of the history.
Having linear dynamics makes it easy to expand terms backwards in time: 
...
Similarly, we can also include a constant term $v_\step \in \mathbb{R}^{n_\st}$ in the dynamics.
This is *deterministic*,
unlike the stochastic noise $w_\step$:
...
We'll exploit the useful fact that we can take a function that's *locally continuous* around $(\st^\star, \act^\star)$ and
...
This only requires us to be able to *query* the function,
not to have an analytical expression for it,
...
**Exercise:**
Convince yourself that $\widetilde{D}$ is indeed positive definite.
...
![Local linearization might only be accurate in a small region around the point of linearization.](shared/log_taylor.png){width="3in"}

### Iterative LQR {#sec-iterative-lqr}

To address these issues with local linearization,
we'll use an iterative approach,
where we repeatedly linearize around different points to create a *time-dependent* approximation of the dynamics,
and then solve the resulting time-dependent LQR problem to obtain a better policy.
This is known as **iterative LQR** or **iLQR**:
...
1.  Form a time-dependent LQR problem (@sec-time-dep-lqr) around the current candidate trajectory
...
4.  Compute a better candidate trajectory
    by interpolating between the current and proposed actions.
:::

```{mermaid}
%%| fig-width: 6
%%| label: fig-ilqr
%%| fig-cap: iLQR can be phrased in the style of policy iteration (@sec-pi). It alternates between computing a locally linear model around the current policy's trajectories and computing the optimal policy that solves the linear model.
graph LR
...
**Step 1: Form a time-dependent LQR problem.**
At each timestep $\step \in [\Step]$,
we use the techniques from @sec-approx-nonlinear to linearize the dynamics
and quadratize the cost function around $(\bar \st^i_\step, \bar \act^i_\step)$:
...
**Step 2: Compute the optimal policy.**
We solve the time-dependent LQR problem using
the time-dependent algebraic Riccati equation (@def-riccati-time-dependent)
to compute the optimal policy
$\pi^i_0, \dots, \pi^i_{\Step-1}$.

**Step 3: Generate a new series of actions.**
We generate a new sample trajectory by taking actions according to this optimal policy:
...
Note that we've denoted these actions as $\widetilde \act_\step$
and aren't directly using them for the next iteration $\bar \act^{i+1}_\step$.
Rather, we want to *interpolate* between them and the actions from the previous iteration
$\bar \act^i_0, \dots, \bar \act^i_{\Step-1}$.
This is so that the cost will *increase monotonically,*
since if the new policy turns out to actually be worse,
we can stay closer to the previous trajectory.
The new policy might be worse,
for example,
if the states in the new trajectory
are far enough from the original states
that the local linearization is no longer accurate.

Formally,
we want to find $\alpha \in [0, 1]$
to generate the next iteration of actions
$\bar \act^{i+1}_0, \dots, \bar \act^{i+1}_{\Step-1}$
...
is a policy $\pi^{n_\text{steps}}$ derived after $n_\text{steps}$ of the algorithm.
Though the proof is somewhat complex,
one can show that for many nonlinear control problems,
this solution converges to a locally optimal solution (in the policy space).


## Key takeaways

This chapter introduced some approaches to solving simple variants of
the optimal control problem @def-optimal-control.
We began with the simple case of linear dynamics and an upward-curved quadratic cost.
This model is called the linear quadratic regulator (LQR).
The optimal policy can be solved using dynamic programming.
We then extended these results to the more general nonlinear case via local linearization.
We finally studied the iterative LQR algorithm for solving nonlinear control problems.

In the big picture,
the LQR algorithm can be seen as an extension of the dynamic programming algorithms in @sec-mdps
to continuous state and action spaces.
Both of these methods assume that the environment is fully known.
iLQR loosens this assumption slightly:
as long as we have "query access" to the environment,
that is,
we can efficiently sample the state following any given state-action pair,
we can locally approximate the dynamics using finite differences.



## Bibliographic notes and further reading {#sec-control-bib}

The field of control theory generally predates that of RL
and can be seen as a precursor to the model-based planning algorithms
used for RL tasks.
People have attempted to solve the optimal control problem
since antiquity:
the first recorded control device (i.e. our notion of a policy,
put into practice)
is thought to be a water clock from the third century BCE
that kept time by controlling the amount of water
flowing out of a vessel [@mayr_populations_1970; @keviczky_control_2019].

Watt's 1788 centrifugal governor
for controlling the pace of a steam engine
was one of the greatest inventions in control at the time.
We refer the reader to [@macfarlane_development_1979; @bellman_adaptive_1961]
for more on the history of control systems.
...
Other researchers also studied the stability of differential equations.
The work of @lyapunov_general_1892 has been widely influential in this field;
his seminal work did not appear in English until @ljapunov_general_1992.

A number of excellent textbooks have been published
on optimal control [@bellman_dynamic_1957; @athans_optimal_1966; @berkovitz_optimal_1974; @lewis_optimal_2012].
We refer interested readers to these for further reading,
particularly about the continuous-time case.


::: {.content-visible when-profile="thesis"}
## Contributions

The presentation of material in this chapter follows the same flow that it did in the course.
I decided to move the derivation of the LQR solution to the appendix
due to student feedback that the linear algebra derivations were
dense and not especially insightful,
though I leave the discrete-time algebraic Riccati equation in the main text
to define the actual recursion.
I also include an example of LQR for the double integrator,
a canonical dynamical system in control theory.

Teaching about optimal control and LQR
is fairly unique to our course
among other RL courses;
a discussion at this level of detail is absent from
the textbooks mentioned in @sec-intro-bib.
I strongly believe that our inclusion of this material is well-justified:
it ties in as a natural extension of the planning algorithms in @sec-mdps
and highlights the efficiency of planning approaches
when the environment is known (or a model can be easily approximated).
It also gives students more exposure to linear algebra
and continuous environments.

The bibliographic notes give a brief history of control systems and control theory
and include pointers to optimal control textbooks.
:::
