...
$\mathcal{S}$ and $\mathcal{A}$ were finite, then we'd be able to work backwards using the DP algorithm for computing the optimal policy in an MDP (@def-pi_star_dp).
...
The optimal control problem @def-optimal_control seems highly complex in general. Is there a relevant simplification that we can analyze?
...
The LQR problem is a special case of the @def-optimal_control with *linear dynamics* and an *upward-curved quadratic cost function*.
...
as we did in @sec-opt_dynamic_programming for MDPs.
...
Q-function $Q^\star_h$,
and policy $\pi^\star_h$ in @def-lqr_definition using **dynamic programming**
in a very similar way to the DP algorithms in @sec-eval_dp.
Recall the definition of the optimal value function:

::: {#def-optimal_value_lqr}
...
::: {#thm-optimal_value_lqr_quadratic}
...
The construction (and inductive proof) proceeds similarly to the one in the MDP setting (@sec-eval_dp).
...
Since $R$ is s.p.d. (@def-lqr_definition),
...
We can simplify the quadratic term by substituting in $K_\hi$ from @eq-k_pi.
...
3. Together with the dynamics given by $A$ and $B$,
   ...
   it fully defines the optimal policy @lem-pi_linear.
...
proving the inductive hypothesis and completing the proof of @thm-optimal_policy_lqr_linear and @thm-optimal_value_lqr_quadratic.
...
that can be defined recursively using @def-riccati.
...
:::{attention} Exercise
Show that if we choose actions according to the optimal policy @lem-pi_linear, @eq-expected_state becomes
...
:::{attention} Exercise
Walk through the proof in @sec-optimal_lqr to verify that we can simply add $\hi$ for the time-dependent case.
:::
...
function above @eq-general_quadratic_cost:
...
(@exm-cart_pole). The dynamics (physics) aren't linear. How
...
problem (see
@def-optimal_control). Here, we don't know an analytical form
...
### Iterative LQR {#sec-iterative_lqr}
...
2. Compute the optimal policy using @sec-time_dep_lqr.
...
@sec-time_dep_lqr to compute the optimal policy
...
@def-optimal_control. We began with the simple case of linear
