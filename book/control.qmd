...
![Cart pole](shared/cart_pole.png){width=200}
...
$\mathcal{S}$ and $\mathcal{A}$ were finite, then we'd be able to work backwards using the DP algorithm for computing the optimal policy in an MDP (@fig-optimal-policy).
...
The optimal control problem @def-optimal-control seems highly complex in general. Is there a relevant simplification that we can analyze?
...
The LQR problem is a special case of the @def-optimal-control with *linear dynamics* and an *upward-curved quadratic cost function*.
...
as we did in @sec-finite-opt-dp for MDPs.
...
Q-function $Q^\star_h$,
and policy $\pi^\star_h$ in @def-lqr-definition using **dynamic programming**
in a very similar way to the DP algorithms in @sec-eval-dp-finite-horizon.
...
The construction (and inductive proof) proceeds similarly to the one in the MDP setting (@sec-eval-dp-finite-horizon).
...
Since $R$ is s.p.d. (@def-lqr-definition),
...
3. Together with the dynamics given by $A$ and $B$,
   ...
   it fully defines the optimal policy @lem-pi-linear.
...
(@exm-cart-pole). The dynamics (physics) aren't linear. How
...
problem (see
@def-optimal-control). Here, we don't know an analytical form
...
@eq-general-quadratic-cost. Derive the corresponding
