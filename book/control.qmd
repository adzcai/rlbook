...
The LQR problem is a special case of the @optimal_control with *linear dynamics* and an *upward-curved quadratic cost function*.
...
Q-function $Q^\star_h$,
and policy $\pi^\star_h$ in @lqr_definition using **dynamic programming**
in a very similar way to the DP algorithms in @eval_dp.
...
Since $R$ is s.p.d. (@lqr_definition),
