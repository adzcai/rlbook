...
This scales *exponentially* in $|\S|$ and $\hor$, which quickly becomes intractable. Notably, this method doesn't consider the information that we gain across different policies. We can illustrate this with the following example:
...
We assume that the transition probabilities and rewards are *linear* in some feature vector $\phi(s, a) \in \R^d$: $$\begin{aligned}
...
The LinUCB-VI algorithm achieves expected regret $$\E[\text{Regret}_T] = \E\left[\sum_{t=0}^{T-1} \Vopt_0(s_0) - V^{\pi^t}_0(s_0) \right] \le \tilde O(H^2 d^{1.5} \sqrt{T})$$
:::
